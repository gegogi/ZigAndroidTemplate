...
pub const AndroidApp = struct {
    ...
    pub fn onNativeWindowCreated(self: *Self, window: *android.ANativeWindow) void {
        ...
        self.screen_width = @floatFromInt(android.ANativeWindow_getWidth(window));
        self.screen_height = @floatFromInt(android.ANativeWindow_getHeight(window));
    ...
    fn processKeyEvent(self: *Self, event: *android.AInputEvent) !bool {
        const event_type: android.AKeyEventActionType = @enumFromInt(android.AKeyEvent_getAction(event));
    ...
    fn insertPoint(self: *Self, point: TouchPoint) void {
        ...
        if (point.index) |index| {
            self.simple_synth.oscillators[@intCast(index)].setWaveOn(true);
        }
    ...
    fn processMotionEvent(self: *Self, event: *android.AInputEvent) !bool {
        const event_type: android.AMotionEventActionType = @enumFromInt(android.AMotionEvent_getAction(event));
            ...
            // Input process
            {
                ...
                if (self.input) |input| {
                    ...
                    while (android.AInputQueue_getEvent(input, &event) >= 0) {
                        ...
                        const event_type: android.AInputEventType = @enumFromInt(android.AInputEvent_getType(event));
            ...
            // Render process
            {
                ...
                if (self.egl) |egl| {
                    ...
                    if (self.egl_init) {
                        ...
                        vPosition = @intCast(vPosition_res);

                        // Bind the vertices to the buffer
                        c.glGenBuffers(1, &touch_buffer);
                        c.glBindBuffer(c.GL_ARRAY_BUFFER, touch_buffer);
                        c.glBufferData(c.GL_ARRAY_BUFFER, @intCast(vVertices[0..].len * @sizeOf(c.GLfloat)), vVertices[0..], c.GL_STATIC_DRAW);
                    ...
                    c.glVertexAttribPointer(vPosition, 2, c.GL_FLOAT, c.GL_FALSE, 0, @ptrFromInt(0));
                    ...
                    for (&self.touch_points) |*pt| {
                        if (pt.*) |*point| {
                            ...
                            if (point.intensity <= 0.0) {
                                if (point.index) |index| {
                                    self.simple_synth.oscillators[@intCast(index)].setWaveOn(false);
                                }
                                pt.* = null;
                            }
                        }
                    ...
                    c.glVertexAttribPointer(mesh_vPosition, 3, c.GL_FLOAT, c.GL_FALSE, @sizeOf(MeshVertex), @ptrFromInt(@offsetOf(MeshVertex, "pos")));

                    c.glEnableVertexAttribArray(mesh_vNormal);
                    c.glVertexAttribPointer(mesh_vNormal, 3, c.GL_FLOAT, c.GL_FALSE, @sizeOf(MeshVertex), @ptrFromInt(@offsetOf(MeshVertex, "normal")));
...
const Vector4 = extern struct {
    ...
    fn readFromSlice(slice: []const u8) Vector4 {
        return Vector4{
            .x = @bitCast(std.mem.readInt(u32, slice[0..4], .little)),
            .y = @bitCast(std.mem.readInt(u32, slice[4..8], .little)),
            .z = @bitCast(std.mem.readInt(u32, slice[8..12], .little)),
            .w = 1.0,
        };
    }
};
...
pub fn glProgramInfoLog(program: c.GLuint) void {
    ...
    if (size == 0) return;
    app_log.info("{s}", .{buffer[0..@intCast(size)]});
}
...
pub fn debugMessageCallback(
    ...
    const message = message: {
        if (message_c) |message_ptr| {
            break :message if (length > 0) message_ptr[0..@intCast(length)] else "";
        } else {
            break :message "";
        }
    };
...
const Oscillator = struct {
    ...
    fn renderf32(self: *@This(), audio_data: []f32) void {
        ...
        for (audio_data) |*frame| {
            if (@atomicLoad(bool, &self.isWaveOn, .seq_cst)) {
                frame.* += @floatCast(.math.sin(self.phase) * self.amplitude);
    ...
    fn renderi16(self: *@This(), audio_data: []i16) void {
        ...
        for (audio_data) |*frame| {
            if (@atomicLoad(bool, &self.isWaveOn, .seq_cst)) {
                frame.* +|= @intFromFloat(@as(f32, @floatCast(std.math.sin(self.phase) * self.amplitude)) * std.math.maxInt(i16));
                self.phase += self.phaseIncrement;
                if (self.phase > std.math.tau) self.phase -= std.math.tau;
            }
        }
...
const SimpleSynth = struct {
    ...
    fn audioCallback(stream: audio.StreamLayout, user_data: *anyopaque) void {
        var synth: *SimpleSynth = @ptrCast(@alignCast(user_data));
