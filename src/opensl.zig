...
pub const OpenSL = struct {
    ...
    pub const OutputStream = struct {
        ...
        pub fn start(output_stream: *OutputStream) !void {
            ...
            try checkResult(output_stream.player.*.*.GetInterface.?(
                ...
                c.SL_IID_PLAY,
                @ptrCast(&output_stream.play_itf),
            ));
            ...
            try checkResult(output_stream.player.*.*.GetInterface.?(
                ...
                @ptrCast(&output_stream.buffer_queue_itf),
            ));
            ...
            while (i < output_stream.config.buffer_count) : (i += 1) {
                try checkResult(output_stream.buffer_queue_itf.*.*.Enqueue.?(
                    ...
                    @intCast(output_stream.config.buffer_size.? * (output_stream.pcm.containerSize / 8)),
                ));
    ...
    pub fn bufferQueueCallback(queue_itf: c.SLAndroidSimpleBufferQueueItf, user_data: ?*anyopaque) callconv(.C) void {
        var output_stream: *OutputStream = @ptrCast(@alignCast(user_data));

        // Lock the mutex to prevent race conditions
        output_stream.mutex.lock();
        defer output_stream.mutex.unlock();

        const buffer = output_stream.buffer[output_stream.buffer_index .. output_stream.buffer_index + output_stream.config.buffer_size.?];
        ...
        checkResult(queue_itf.*.*.Enqueue.?(
            queue_itf,
            @ptrCast(buffer.ptr),
            @intCast((output_stream.pcm.containerSize / 8) * buffer.len),
        )) catch |e| {
    ...
    pub fn init() SLError!void {
        ...
        if (init_sl_counter == 1) {
            ...
            try checkResult(engine_object.*.*.GetInterface.?(engine_object, c.SL_IID_ENGINE, @ptrCast(&engine)));
            ...
            try checkResult(output_mix.*.*.GetInterface.?(output_mix, c.SL_IID_OUTPUTMIX, @ptrCast(&output_mix_itf)));
        }
    }
    ...
    pub fn getOutputStream(allocator: std.mem.Allocator, conf: OutputStreamConfig) !*OutputStream {
        ...
        const buffers = try allocator.alloc(i16, config.buffer_size.? * config.buffer_count);
        ...
        output_stream.* = OutputStream{
            ...
            .buffer_queue = .{
                ...
                .numBuffers = @intCast(config.buffer_count),
            },
            .pcm = .{
                .formatType = c.SL_DATAFORMAT_PCM,
                .numChannels = @intCast(config.channel_count),
            ...
            .audio_source = .{
                .pFormat = @ptrCast(&output_stream.pcm),
                .pLocator = @ptrCast(&output_stream.buffer_queue),
            },
            .locator_outputmix = .{
            ...
            .audio_sink = .{
                .pLocator = @ptrCast(&output_stream.locator_outputmix),
                .pFormat = null,
            },
    ...
    fn printEngineExtensions() !void {
        ...
        try checkResult(engine.*.*.QueryNumSupportedExtensions.?(engine, &extension_count));
        {
            ...
            while (i < extension_count) : (i += 1) {
                ...
                const extension_name = extension_ptr[0..@intCast(extension_size)];
