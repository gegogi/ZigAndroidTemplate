...
pub const AAudio = struct {
    ...
    fn dataCallback(
        ...
        _ = stream;
        const output_stream: *OutputStream = @ptrCast(@alignCast(user_data.?));
        // TODO:
        // const audio_slice = @as([*]f32, @ptrCast(@alignCast(audio_data.?)))[0..@intCast(num_frames)];
        const audio_slice = @as([*]i16, @ptrCast(@alignCast(audio_data.?)))[0..@intCast(num_frames)];
        ...
        const stream_layout = StreamLayout{
            ...
            .channel_count = @intCast(output_stream.config.channel_count),
    ...
    pub fn getOutputStream(allocator: std.mem.Allocator, config: OutputStreamConfig) !*OutputStream {
        ...
        c.AAudioStreamBuilder_setChannelCount(stream_builder, @intCast(config.channel_count));
        ...
        if (config.sample_rate) |rate| c.AAudioStreamBuilder_setSampleRate(stream_builder, @intCast(rate));
        if (config.buffer_size) |size| c.AAudioStreamBuilder_setFramesPerDataCallback(stream_builder, @intCast(size));
        ...
        output_stream.config.sample_rate = @intCast(c.AAudioStream_getSampleRate(output_stream.stream));
        output_stream.config.buffer_size = @intCast(c.AAudioStream_getFramesPerBurst(output_stream.stream));

        const res = c.AAudioStream_setBufferSizeInFrames(output_stream.stream, @intCast(output_stream.config.buffer_count * output_stream.config.buffer_size.?));
        if (res < 0) {
